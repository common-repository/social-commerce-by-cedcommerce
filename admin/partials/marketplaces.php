...
if ( is_array( $active_marketplaces ) && ! empty( $active_marketplaces ) ) {
	...
	<div class="ced-marketplaces-heading-main-wrapper">
		<div class="ced-marketplaces-heading-wrapper">
			<h2><?php esc_html_e( 'Active Marketplaces', 'facebook-marketplace-connector-for-woocommerce' ); ?></h2>
		</div>
		...
		foreach ( $active_marketplaces as $key => $value ) {
			...
			<div class="ced-marketplace-card <?php echo esc_attr( $value['name'] ); ?>">
				<a href="<?php echo esc_url( $url ); ?>">
					<div class="thumbnail">
						<div class="thumb-img">
							<img class="img-responsive center-block integration-icons" src="<?php echo esc_url( $value['card_image_link'] ); ?>" height="auto" width="auto" alt="Get Your Products Listed on Facebook">
						</div>
					</div>
